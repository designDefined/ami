...
const CleanParser = ({ markDown }: Props) => {
  ...
  switch (type) {
    case "p":
      return <p className={cx("p")}>{innerText}</p>;
    case "h1":
      return <h1 className={cx("h1")}>{innerText}</h1>;
    case "h2":
      return <h2 className={cx("h2")}>{innerText}</h2>;
    case "h3":
      return <h3 className={cx("h3")}>{innerText}</h3>;
    case "h4":
      return <h4 className={cx("h4")}>{innerText}</h4>;
    default:
